    ...
    return [
        {
            ...
            plugins: [
                ...
                new html_webpack_plugin_1({
                    template: path_1.resolve(__dirname, env.src + "/index.html") /* 自动在该模板中导入 output 中的filename文件 */
                }),
                new webpack_1.HotModuleReplacementPlugin(),
                new webpack_1.ProgressPlugin() /* 控制台显示加载进度 */,
            ]
        },
    ];
}
function devServer(compiler, env) {
    return new webpack_dev_server_1(compiler, {
...
function start(env) {
    ...
    server.listen(env.port, "0.0.0.0", function (error) {
        ...
        console.info(chalk_1(templateObject_1 || (templateObject_1 = __makeTemplateObject(["starting dev server on {green ", "://localhost:", "/} \n"], ["starting dev server on {green ", "://localhost:", "/} \\n"])), env.https ? "https" : "http", env.port));
        return null;
    });
